...
entity Main_FSM_Counter is
    ...
    Port (
        i_clk         : in  STD_LOGIC;
        i_res         : in  STD_LOGIC;
        i_en_2hz      : in  STD_LOGIC; -- 適用於 2Hz 訊號的計數器
        ...
        o_restart_div : out STD_LOGIC; -- 重新觸發除頻器
...
begin
    ...
    -- 下一個狀態邏輯
    FSM:process(current_state, i_en, i_load, i_up_down)
    begin
        -- 預設保持當前狀態
    ...
    -- 輸出邏輯與觸發除頻器
    counter:process(i_clk, i_res)
    begin
        if i_res = '1' then
        elsif rising_edge(i_clk) then
            o_restart_div <= '0'; -- 預設不觸發除頻器
            
            -- 狀態轉換時，觸發除頻器 (對應 IDLE 到 COUNT 或 UP/DOWN 的轉換)
            if next_state /= current_state then
                o_restart_div <= '1'; -- 觸發除頻器
            end if;
