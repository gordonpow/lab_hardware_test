...
## Universal Visual and Layout Rules
Every output generated by these prompts must follow these strict style constraints:
1. Canvas Background: Must be solid black (background="#000000").
2. No Overlapping Edges: Connections (lines) must NEVER pass through or be blocked by any rectangles, circles, or text labels. Precise terminal points (exitX/Y, entryX/Y) must be defined, and orthogonal routing points (points) must be used to bypass components.
3. Contrast-Aware Typography:
   - Default text color must be white (fontColor=#FFFFFF).
   - If a node has a light-colored background (e.g., yellow #FFFF00, cyan #00FFFF), the text color of that node must be changed to black (fontColor=#000000) for readability.
   - Any labels on edges must have a solid background color (labelBackgroundColor) matching the edge color to guarantee contrast on the black canvas.

---

## 1. Breakdown Chart (Module Hierarchy Diagram)
...
## 2. RTL Architecture and Dataflow Diagram
...
## 3. Finite State Machine (FSM) Transition Diagram
...
## 4. Activity On Vertex (AOV) Transition Diagram
...
## 5. Message Sequence Chart (MSC / Waveform Timing Diagram)
