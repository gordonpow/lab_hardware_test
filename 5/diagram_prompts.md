...
2. 分層與配色規則：
   ...
   - 同一個層級中的各個子模組 (方塊) 必須各自分配完全不同、明亮且高對比的填充顏色 (同一層不同顏色，例如同一層的四個方塊可分別使用橘色、青色、黃色、綠色)。
   - 對於每一層的節點樣式：
     - 第 1 層 (頂層)：矩形，無圓角 (rounded=0;whiteSpace=wrap;html=1;)。
       - 填充色：預設為顯眼的單一主色 (如紅色 fillColor=#FF0000)
       - 邊框色：自訂顏色的深色版 (strokeColor=#000000)
       - 字體色：白色 (fontColor=#ffffff)
       - 建議尺寸較大：width="120" height="40"
     - 第 2 層及更深層 (子模組/子電路)：矩形，無圓角 (rounded=0;whiteSpace=wrap;html=1;)。
       - 填充色：該層內部的各個節點必須各自使用完全不同的亮色 (如 fillColor=橘色、青色、黃色、綠色等)
       - 邊框色：黑色 (strokeColor=#000000)
       - 字體色：若填充色偏淡 (如黃色、青色)，字體顏色必須設為黑色 (fontColor=#000000)；若填充色偏深，字體顏色設為白色 (fontColor=#ffffff)。
       - 建議尺寸：width="80" height="40" (或依字數調整)
3. 連線樣式與避空規則：
