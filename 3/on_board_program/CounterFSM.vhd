...
architecture Behavioral of CounterFSM is
    ...
    signal r_Cnt1 : UNSIGNED(DATA_WIDTH -1 downto 0);
    signal r_Cnt2 : UNSIGNED(DATA_WIDTH -1 downto 0);
        ...
        if i_rst = '1' then
            ...
            if i_en = '0' or CurrentState = Idle then 
                ...
                if CurrentState = Cnt1Count then
                    ...
                    if r_Cnt1 >= unsigned(i_Cnt1_lim_up) then
                        r_Cnt1 <= (others => '0');
                        r_Cnt2 <= to_unsigned(1, DATA_WIDTH); -- Start Cnt2 immediately
                    else
                         r_Cnt1 <= r_Cnt1 + 1;
                         r_Cnt2 <= (others => '0');
                    end if;
                    ...
                    if r_Cnt2 >= unsigned(i_Cnt2_lim_up) then
                         r_Cnt2 <= (others => '0');
                         r_Cnt1 <= to_unsigned(1, DATA_WIDTH); -- Start Cnt1 immediately
                    else
                         r_Cnt2 <= r_Cnt2 + 1;
                         r_Cnt1 <= (others => '0');
                    end if;
